...
for passport in input.split('\n\n'):
    valid_passport_count += int(
        set([field.split(':')[0] for field in passport.split()]).issuperset(mandatory_passport_fields))
print(valid_passport_count)

# Part-2
valid_passport_count = 0
for passport in input.split('\n\n'):
    if not set([field.split(':')[0] for field in passport.split()]).issuperset(mandatory_passport_fields):
        continue
    ...
    for field in passport.split():
        ...
        if key == 'byr':
            ...
            if val.endswith('cm'):
                if not (150 <= int(val.split('cm')[0]) <= 193):
                    continue
            elif val.endswith('in'):
                if not (59 <= int(val.split('in')[0]) <= 76):
                    continue
            else:
                continue
        elif key == 'hcl':
            if not re.compile("^#[a-f0-9]{6}$").match(val):
                continue
        elif key == 'ecl':
            if val not in ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]:
                continue
        elif key == 'pid':
            if not re.compile("^[0-9]{9}$").match(val):
                continue
        else:
            continue

        valid_field_count += 1
    valid_passport_count += (valid_field_count == len(mandatory_passport_fields))
